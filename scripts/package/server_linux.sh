#!/usr/bin/env bash
# Package Plix headless server for Linux
#
...
#
# Usage:
#   ./server_linux.sh --binary-path <path> --version <semver> --output-dir <path>
#
# Exit codes:
#   0 - Success
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Package Plix headless server for Linux"
            echo ""
...
echo "Packaging plix-server-headless for Linux..." >&2
...
if [[ -d "$PROJECT_ROOT/deploy/configs/examples" ]]; then
    ...
    # Create minimal example configs
...
if [[ ! -f "$BUNDLE_DIR/configs/examples/server.toml" ]]; then
    echo "Warning: Example config not found" >&2
fi
