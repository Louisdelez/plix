...
impl Server {
    ...
    pub async fn new(config: ServerConfig) -> Result<Self, ServerError> {
        ...
        let match_state = MatchStateMachine::new(match_config.clone());
    ...
    async fn tick(&mut self) -> Result<(), ServerError> {
        ...
        if let Some(new_phase) = self.match_state.update(self.current_tick, player_count) {
    ...
    async fn simulate_tick(&mut self) {
        ...
        let respawn_delay = self.match_config.respawn_delay;
                ...
                if let Some(respawn_tick) = player.respawn_tick {
                    if current_tick.0 >= respawn_tick.0 {
                        ...
                        if let Some(spawn) = self.spawn_manager.get_spawn_point(player.team) {
                            player.spawn(spawn.position.into(), spawn.rotation);
        ...
        for (attacker_id, attacker_pos, attacker_forward, last_attack_tick) in attack_requests {
            ...
            let targets: Vec<(PlayerId, Vec3, u8)> = self
                .sessions
                .iter()
                .filter(|p| !p.is_dead && p.id != attacker_id)
    ...
    async fn send_snapshots(&mut self) -> Result<(), ServerError> {
        ...
        // Send to each player
        let player_addrs: Vec<(SocketAddr, PlayerId, InputSeq)> = self
            .sessions
            .iter()
            .map(|p| (p.addr, p.id, p.last_input_seq))
            .collect();

        for (addr, _player_id, last_input_seq) in player_addrs {
